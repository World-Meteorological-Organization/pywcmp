...
class WCMP2ETSTest(unittest.TestCase):
    ...
    def test_fail_invalid_link_channel_wis2_topic_wss(self):
        """
        Simple tests for a failing record with an invalid
        link channel WIS2 topic when using WebSockets
        """

        with open(get_test_file_path('data/wcmp2-failing-invalid-link-channel-wis2-topic-wss.json')) as fh:  # noqa
            record = json.load(fh)
            ts = WMOCoreMetadataProfileTestSuite2(record)
            results = ts.run_tests()
